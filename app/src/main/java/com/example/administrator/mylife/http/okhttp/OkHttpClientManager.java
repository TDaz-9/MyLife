package com.example.administrator.mylife.http.okhttp;
...
public class OkHttpClientManager {
    ...
    public void execute(final Request request, ResultCallback callback) {
        ...
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(final Request request, final IOException e) {
                ...
                L.e("test", "response error==" + e.getMessage());
                sendFailResultCallback(request, e, resCallBack);
            }

            @Override
            public void onResponse(final Response response) {
                L.i("test", "response  Code==" + response.code());
                ...
                try {
                    ...
                    L.i("test", "response  Data==" + string);
                    BaseResp baseResp = mGson.fromJson(string, BaseResp.class);
                    if (null != baseResp && !CommomMethod.isEmpty(baseResp.getReason())) {
                        L.i("test", "提示信息==" + baseResp.getReason());
                    }
                    if (null != baseResp && !baseResp.isSuccess()) {
                        postSerCallback(resCallBack, baseResp);
                        return;
                    }
    ...
    public void postSerCallback(final ResultCallback callback, final BaseResp baseResp) {
        if (callback == null) return;
        mDelivery.post(new Runnable() {
            @Override
            public void run() {
                callback.postSerMsg(baseResp);
                callback.onAfter();
            }
        });
    }
